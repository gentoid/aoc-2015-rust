...
fn parse_line(input: &str) -> (String, String, i32) {
    let temlate = Regex::new(r"(\w+) would (\w+) (\d+) happiness units by sitting next to (\w+)\.").unwrap();
    ...
    (captures[1].to_owned(), captures[4].to_owned(), sign * captures[3].parse::<i32>().unwrap())
}
...
mod tests {
    ...
    fn parses_lines() {
        ...
        for (input, (person1, person2, happiness)) in data {
            assert_eq!(parse_line(input), (person1.to_owned(), person2.to_owned(), happiness));
        }
    }
}
